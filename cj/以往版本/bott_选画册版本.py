...
dir_path="C:\\Users\\Win\\Desktop\\cj\\图片素材\\搜索库\\"
                        ...
                        if (ev.type=='primary'):
                            ...
                            if(ev.message=="十连"):
                                ...
                                if ev.group_id in dic_group:
                                    ...
                                    l_p=[i for i in all_pic_d[fo] if i not in group_pic_d[ev.group_id][fo]]
                                    if (l_p==all_pic_d[fo]):
                                        #没懂
                                        cur.execute("insert into PIC_NOW_GROUP(groupid,forder_n) values (%s,'%s')" %(str(ev.group_id),fo))
                                        conn_p.commit()
                                    if l_p==[]:
                                        #如果此文件夹为空，则重选，但这会有问题
                                        folder_now_d[ev.group_id]=random.choice([i for i in all_pic_d.keys() if i not in group_pic_d[ev.group_id].keys()])
                                        fo=folder_now_d[ev.group_id]
                                        #文件夹选取有误
                                        l_p=[i for i in all_pic_d[fo] if i not in group_pic_d[ev.group_id][fo]]
                                        cur.execute("update PIC_NOW_GROUP set forder_n='%s' where groupid=%s" %(fo,str(ev.group_id)))
                                        conn_p.commit()
                                    one_pic=random.choice(l_p)
                                    one_p_path=dir_path+"\\"+fo+"\\"+one_pic
                                    group_pic_d[ev.group_id][fo].append(one_pic)
                                    b.send_group_msg(ev.group_id,'image',url="file:///"+one_p_path)
                                ...
                                l_change=re.compile(r'凯露切换画夹为 (.*)').findall(msg)
                                if l_change!=[]:
                                    auther=l_change[0]
                                    if auther in all_pic_d.keys():
                                        folder_now_d[ev.group_id]=auther
                                        self.send_group_msg(ev.group_id,'text','目前画夹为'+folder_now_d[ev.group_id])
...
def threading_chou_up(b:bot,ev=event):
    ...
    url='file:///C:\\Users\\Win\\Desktop\\抽up.png'
...
def chou_up():
    ...
    l_p=[]
    fo='三星'
    l_p.append("C:\\Users\\Win\\Desktop\\cj\\图片素材\\%s_加框\\%s_加框.jpg" %(fo,role))
    if l!=[]:
        out1+="三星角色总览:\n"
        for i in l:
            l_p.append("C:\\Users\\Win\\Desktop\\cj\\图片素材\\%s_加框\\%s_加框.jpg" %(fo,str(i)))
